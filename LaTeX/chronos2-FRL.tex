...
The paper’s contribution is fourfold. First, we provide a systematic comparison of MV versus UV forecasting via a consistent rolling evaluation, finding that MV outperforms UV. Second, we examine if MV gains differ by asset class and if joint modeling improves accuracy. We find that adding different asset classes does not improve forecasts, highlighting a signal-to-noise tradeoff where expanding the basis set is not always beneficial. Third, we offer large-scale outputs—error metrics, uncertainty quantiles, and visualizations—to support robust evaluation. Fourth, Appendix \ref{ai_workflow} details how AI was used to produce this paper. Collectively, these results clarify when foundation models add value to financial forecasting.
